...
const DB_FILE = path.join(config.DATA_DIR, 'kardex.db');
...
function init() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS employees (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      cedula TEXT, nombres TEXT NOT NULL DEFAULT '', apellidos TEXT NOT NULL DEFAULT '',
      sexo TEXT NOT NULL DEFAULT '', fecha_nacimiento TEXT NOT NULL DEFAULT '',
      nacionalidad TEXT NOT NULL DEFAULT '', lugar_nacimiento TEXT NOT NULL DEFAULT '',
      estado_civil TEXT NOT NULL DEFAULT '', profesion TEXT NOT NULL DEFAULT '',
      tipo_sangre TEXT NOT NULL DEFAULT '', puesto TEXT NOT NULL DEFAULT '',
      departamento TEXT NOT NULL DEFAULT '', sucursal TEXT NOT NULL DEFAULT '',
      fecha_emision TEXT NOT NULL DEFAULT '', fecha_vencimiento TEXT NOT NULL DEFAULT '',
      nota TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'activo',
      es_propietario INTEGER NOT NULL DEFAULT 0,
      fecha_baja TEXT NOT NULL DEFAULT '', salario REAL NOT NULL DEFAULT 0,
      tipo_salario TEXT NOT NULL DEFAULT 'mensual', fecha_ingreso TEXT NOT NULL DEFAULT '',
      nss TEXT NOT NULL DEFAULT '', ars TEXT NOT NULL DEFAULT '', afp TEXT NOT NULL DEFAULT '',
      email TEXT NOT NULL DEFAULT '', telefono TEXT NOT NULL DEFAULT '',
      flota TEXT NOT NULL DEFAULT '', banco TEXT NOT NULL DEFAULT '',
      cuenta TEXT NOT NULL DEFAULT '', tipo_contrato TEXT NOT NULL DEFAULT '',
      foto TEXT, frente TEXT, reverso TEXT,
      ciudad TEXT NOT NULL DEFAULT '',
      created_by INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS vacaciones (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, tipo TEXT NOT NULL DEFAULT 'vacaciones',
      fecha_inicio TEXT NOT NULL DEFAULT '', fecha_fin TEXT NOT NULL DEFAULT '',
      dias REAL NOT NULL DEFAULT 0, motivo TEXT NOT NULL DEFAULT '',
      aprobado INTEGER NOT NULL DEFAULT 1,
      modalidad TEXT NOT NULL DEFAULT 'tomadas',
      dias_pagados REAL NOT NULL DEFAULT 0, dias_guardados REAL NOT NULL DEFAULT 0,
      created_at TEXT NOT NULL, created_by INTEGER
    );
    CREATE TABLE IF NOT EXISTS horas_extra (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, mes INTEGER NOT NULL, anio INTEGER NOT NULL,
      horas_extra REAL NOT NULL DEFAULT 0, domingos_extra INTEGER NOT NULL DEFAULT 0,
      feriados_extra REAL NOT NULL DEFAULT 0, otros_ingresos REAL NOT NULL DEFAULT 0, transporte REAL NOT NULL DEFAULT 0,
      deducciones REAL NOT NULL DEFAULT 0, nota TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS incentivos (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, mes INTEGER NOT NULL, anio INTEGER NOT NULL,
      monto REAL NOT NULL DEFAULT 0, motivo TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by INTEGER
    );
    CREATE TABLE IF NOT EXISTS pago_vacaciones (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, mes INTEGER NOT NULL, anio INTEGER NOT NULL,
      dias REAL NOT NULL DEFAULT 0, monto REAL NOT NULL DEFAULT 0,
      modalidad TEXT NOT NULL DEFAULT 'personalizada', nota TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by INTEGER
    );
    CREATE TABLE IF NOT EXISTS liquidaciones (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, fecha_baja TEXT NOT NULL DEFAULT '',
      salario_mensual REAL NOT NULL DEFAULT 0, salario_diario REAL NOT NULL DEFAULT 0,
      tiempo_years INTEGER NOT NULL DEFAULT 0, tiempo_months INTEGER NOT NULL DEFAULT 0,
      tiempo_days INTEGER NOT NULL DEFAULT 0, meses_servicio REAL NOT NULL DEFAULT 0,
      cesantia_dias REAL NOT NULL DEFAULT 0, preaviso_dias REAL NOT NULL DEFAULT 0,
      vacaciones_dias REAL NOT NULL DEFAULT 0, cesantia REAL NOT NULL DEFAULT 0,
      preaviso REAL NOT NULL DEFAULT 0, vacaciones REAL NOT NULL DEFAULT 0,
      regalia REAL NOT NULL DEFAULT 0, total REAL NOT NULL DEFAULT 0,
      ha_sido_preavisado INTEGER NOT NULL DEFAULT 0, incluir_cesantia INTEGER NOT NULL DEFAULT 1,
    ...
    CREATE TABLE IF NOT EXISTS mail_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      to_email TEXT NOT NULL DEFAULT '', subject TEXT NOT NULL DEFAULT '',
    ...
    CREATE TABLE IF NOT EXISTS contactos_externos (
      ...
      notas TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS historial_empleados (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, campo TEXT NOT NULL DEFAULT '',
      valor_anterior TEXT NOT NULL DEFAULT '', valor_nuevo TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL, created_by INTEGER
    );
    CREATE TABLE IF NOT EXISTS deducciones_manuales (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, mes INTEGER NOT NULL, anio INTEGER NOT NULL,
      quincena INTEGER NOT NULL DEFAULT 0, monto REAL NOT NULL DEFAULT 0,
      motivo TEXT NOT NULL DEFAULT '',
      created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by INTEGER
    );
    CREATE TABLE IF NOT EXISTS salario_historial (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      employee_id INTEGER NOT NULL, salario REAL NOT NULL DEFAULT 0,
      salario_anterior REAL NOT NULL DEFAULT 0, tipo_salario TEXT NOT NULL DEFAULT 'mensual',
      fecha_cambio TEXT NOT NULL, motivo TEXT NOT NULL DEFAULT ''
    );
...
// Migración: agrega columnas nuevas a horas_extra si la BD ya existía.
function ensureHorasExtraColumns() {
  ...
  for (const [name, def] of toAdd) {
    if (!cols.has(name)) {
      run(`ALTER TABLE horas_extra ADD COLUMN ${name} ${def}`);
    }
  }
}

// Migración: agrega columnas nuevas a employees si la BD ya existía.
function ensureEmployeeColumns() {
  ...
  for (const [name, def] of toAdd) {
    if (!cols.has(name)) {
      run(`ALTER TABLE employees ADD COLUMN ${name} ${def}`);
    }
  }
}
...
function seed() {
  ...
  if ((row && row.c || 0) === 0) {
    run('INSERT INTO users (username, password_hash, role, full_name, created_at) VALUES (?,?,?,?,?)',
      ['admin', hashPassword('admin123'), 'admin', 'Administrador', nowIso()]);
    console.log('[DB] Usuario admin por defecto creado (admin / admin123)');
  }
}
...
const users = {
  ...
  create({ username, password, role, full_name }) {
    ...
    const created = get('SELECT id FROM users WHERE username=?', [uname]);
...
const employees = {
  ...
  create(data, userId) {
    const d = Object.assign({
      ...
      sucursal: '', fecha_emision: '', fecha_vencimiento: '', nota: '',
    ...
    if (Number(d.salario) > 0) salarioHistorial.record(created.id, Number(d.salario), d.tipo_salario || 'mensual', 'Alta');
...
const salarioHistorial = {
  ...
  resetBaseline() {
    ...
    return { anio: y, base, registros: get('SELECT total_changes() AS n').n };
  },
  getSalarioPromedio(employeeId, anio) {
    ...
    for (const ch of allChanges) {
      if (new Date(ch.fecha_cambio) < inicioAnio) salarioInicio = ch.salario;
    }
...
const vacaciones = {
  list(employeeId) {
    if (employeeId) return all('SELECT * FROM vacaciones WHERE employee_id = ? ORDER BY fecha_inicio DESC', [employeeId]);
  ...
  create(data, userId) {
    ...
    run(
      ...
      [d.employee_id, String(d.tipo), String(d.fecha_inicio || ''), String(d.fecha_fin || ''), Number(d.dias) || 0,
       ...
       String(d.modalidad || 'tomadas'), pagados, guardados, nowIso(), userId || null]);
...
const contactos = {
  list() { return all('SELECT * FROM contactos_externos ORDER BY nombre'); },
  get(id) { return get('SELECT * FROM contactos_externos WHERE id = ?', [id]); },
  create(data) {
    const d = Object.assign({ nombre: '', email: '', notas: '' }, data || {});
    const email = String(d.email || '').trim();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error('Correo no válido');
    run('INSERT INTO contactos_externos (nombre, email, notas, created_at) VALUES (?,?,?,?)',
  ...
  delete(id) { run('DELETE FROM contactos_externos WHERE id = ?', [id]); return true; }
};
...
const backups = {
  ...
  create(auto = false) {
    ...
    return { file: fname, size: fs.statSync(dest).size };
  },
  list() {
    const dir = backups.dir();
  ...
  restore(file) {
    const src = path.join(backups.dir(), path.basename(file));
...
module.exports = {
  ...
  auth, users, employees, vacaciones, horasExtra, incentivos, pagoVacaciones,
  deduccionesManuales, salarioHistorial, liquidaciones, reportes, audit,
  settings, mailLog, contactos, historial, backups, nowIso
};
